        ...
        <table class=" min-w-full">
            <thead>
                <tr>
                    ...
                    <th class="px-6 py-3 border-b-2 border-gray-300 text-left text-sm leading-4 text-blue-500 tracking-wider">Ads Image</th>
...
<tr>
        <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-500">
            <div class="flex items-center">
                <div>
                    <div class="text-sm leading-5 text-gray-800">#{{$ad->id}}</div>
                </div>
            </div>
        </td>

        <td class="px-6 py-4 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5">{{ Str::limit($ad->link, 20)}}</td>


        <td class="px-4 py-2  border-b border-gray-500">
            <div class="flex items-center">
                    <img src="{{ asset($ad->ads) }}" class="w-24 rounded-xl">
            </div>
        </td>
        ...
        <td class="px-4 py-2  border-b border-gray-500">
            <div class="flex items-center">
               @if ($ad->type == 1)
                   <span class="bg-green-500  px-2 py-1 text-white rounded-full font-bold">Vertical</span>
               @else
                    <span class="bg-blue-400 px-2 py-1 text-white rounded-full font-bold">Horizontal</span>
               @endif
            </div>
        </td>


        <td class="px-6 py-4 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5">{{ \Carbon\Carbon::parse($ad->created_at)->diffForHumans() }}

        </td>


        <td class="whitespace-no-wrap text-right border-b border-gray-500 text-sm leading-5">
            <a href="{{ route('admin.ads.edit', $ad->id) }}"><button  type="button" class="font-bold border border-gray-500 bg-yellow-500 text-white rounded-md px-4 py-2 m-2 transition duration-500 ease select-none hover:bg-yellow-600 focus:outline-none focus:shadow-outline">Edit</button></a>
            <form class="inline-flex" action="{{ route('admin.ads.destroy', $ad->id) }}" method="POST">
                @csrf
                @method('DELETE')
