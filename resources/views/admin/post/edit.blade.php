...
<div class="w-full xl:w-2/3 block px-10 bg-white rounded-xl py-5 shadow-md">
    ...
    <div class="flex justify-between py-3 mx-5">
        <span class="text-xl font-semibold text-gray-700">Edit Post</span>
    ...
    <form action="{{ route('admin.post.update', $post) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="text-gray-700 md:flex md:items-center p-4">
            ...
            <div class="md:w-2/3 md:flex-grow">
                <input name="title_en" value="{{$post->title_en}}" class="w-full h-10 px-3 text-sm placeholder-gray-600 border rounded-lg focus:shadow-outline" type="text" placeholder="Enter EN Title" />
                @error('title_en')
        ...
        <div class="text-gray-700 md:flex md:items-center p-4">
            ...
            <div class="md:w-2/3 md:flex-grow">
                <input name="title_mk" value="{{$post->title_mk}}" class="w-full h-10 px-3 text-sm placeholder-gray-600 border rounded-lg focus:shadow-outline" type="text" placeholder="Enter MK Title" />
                @error('title_mk')
                <span class="bg-red-500 px-3 py-1 text-xs text-white font-semibold rounded-full">{{ $message }}</span>
                @enderror
            </div>
            ...
            <label class="md:w-2/3 md:flex-grow">
                <select class="form-select block w-full mt-1 rounded-lg" name="category_id"  >
                    ...
                    @foreach ($categories as $category)
                        <option  class="text-xs md:text-lg" value="{{ $category->id}}"
                            {{ $category->id == $post->category_id ? 'selected' : '' }}
                            >{{ $category->category_en }} | {{ $category->category_mk }}</option>
                    @endforeach
                </select>
                @error('category_id')
                <span class="bg-red-500 px-3 py-1 text-xs text-white font-semibold rounded-full">{{ $message }}</span>
        ...
        <div class="text-gray-700 md:flex md:items-center p-4">
            ...
            <label class="md:w-2/3 md:flex-grow">
                <select id="subcategory_id" class="form-select block w-full mt-1 rounded-lg" name="subcategory_id"  >
                    ...
                    <option  class="text-xs md:text-lg" value="{{ $subcategory->id}}"
                        {{ $subcategory->id == $post->subcategory_id ? 'selected' : '' }}
                        >{{ $subcategory->subcategory_en }} | {{ $subcategory->subcategory_mk }}</option>
                @endforeach
            ...
            <label class="md:w-2/3 md:flex-grow">
                <select class="form-select block w-full mt-1 rounded-lg" name="area_id"  >
                    ...
                    @foreach ($areas as $area)
                        <option  class="text-xs md:text-lg" value="{{ $area->id}}"
                            {{ $area->id == $post->area_id ? 'selected' : '' }}
                            >{{ $area->area_en }} | {{ $area->area_mk }}</option>
                    @endforeach
                </select>
                @error('area_id')
                <span class="bg-red-500 px-3 py-1 text-xs text-white font-semibold rounded-full">{{ $message }}</span>
        ...
        <div class="text-gray-700 md:flex md:items-center p-4">
            ...
            <label class="md:w-2/3 md:flex-grow">
                <select id="subarea_id" class="form-select block w-full mt-1 rounded-lg" name="subarea_id" >
                    ...
                    <option  class="text-xs md:text-lg" value="{{ $subarea->id}}"
                        {{ $subarea->id == $post->subarea_id ? 'selected' : '' }}
                        >{{ $subarea->subarea_en }} | {{ $subarea->subarea_mk }}</option>
                @endforeach
                </select>
                @error('subarea_id')
                <span class="bg-red-500 px-3 py-1 text-xs text-white font-semibold rounded-full">{{ $message }}</span>
        ...
        <div class="text-gray-700 md:flex md:items-center p-4">
            ...
            <div class="md:w-2/3 md:flex-grow">
                <input name="tags_en" value="{{ $post->tags_en }}" class="w-full h-10 px-3 text-sm placeholder-gray-600 border rounded-lg focus:shadow-outline" type="text" placeholder="Enter EN Tags" />
                @error('tags_en')
                <span class="bg-red-500 px-3 py-1 text-xs text-white font-semibold rounded-full">{{ $message }}</span>
                @enderror
            </div>
        ...
        <div class="text-gray-700 md:flex md:items-center p-4">
            ...
            <div class="md:w-2/3 md:flex-grow">
                <input name="tags_mk" value="{{ $post->tags_mk }}" class="w-full h-10 px-3 text-sm placeholder-gray-600 border rounded-lg focus:shadow-outline" type="text" placeholder="Enter MK Tags" />
                @error('tags_mk')
                <span class="bg-red-500 px-3 py-1 text-xs text-white font-semibold rounded-full">{{ $message }}</span>
                @enderror
            </div>
        ...
        <div class="text-gray-700 md:flex md:items-center p-4">
            <div class="mb-1 md:mb-0 md:w-1/3">
                <label class="text-gray-800 font-bold">Current Image</label>
            </div>
            <div class="md:w-2/3 md:flex-grow">
                <img class="h-56" src="{{ asset($post->image) }}" alt="">
        ...
        <div class="text-gray-700 md:flex md:items-center p-4">
            <div class="mb-1 md:mb-0 md:w-1/3">
                <label class="text-gray-800 font-bold">New Image Upload</label>
            </div>
            <div class="md:w-2/3 md:flex-grow">
        ...
        <div class="text-gray-700 md:flex md:items-center p-4">
            ...
            <div class="md:w-2/3 md:flex-grow">
                ...
                type="text">{{ $post->description_en }}</textarea>                
        ...
        <div class="text-gray-700 md:flex md:items-center p-4">
            ...
            <div class="md:w-2/3 md:flex-grow">
                <textarea id="summernote" name="description_mk" class="autoexpand tracking-wide py-2 px-4 mb-3 leading-relaxed appearance-none block w-full bg-gray-200 border border-gray-200 rounded focus:outline-none focus:bg-white focus:border-gray-500"
                 type="text">{{ $post->description_mk }}</textarea>
        ...
        <div class="text-gray-700 md:flex md:items-center p-4">
            ...
            <div class="md:w-2/3 md:flex-grow">
                ...
                <label class="inline-flex items-center p-2">
                    <input name="headline" type="hidden" value="0">
                    <input name="headline" type="checkbox" class="form-checkbox h-5 w-5 text-blue-600" value="1"
                    {{ $post->headline == 1 ? 'checked' : ''}}
                    ><span class="ml-2 text-gray-700">Headline</span>
                </label>

                <label class="inline-flex items-center p-2">
                    <input name="first_section" type="hidden" value="0">
                    <input name="first_section" type="checkbox" class="form-checkbox h-5 w-5 text-blue-600" value="1"
                    {{ $post->first_section == 1 ? 'checked' : ''}}
                    ><span class="ml-2 text-gray-700">First Section</span>
                </label>

                <label class="inline-flex items-center p-2">
                    <input name="first_section_thumbnail" type="hidden" value="0">
                    <input name="first_section_thumbnail" type="checkbox" class="form-checkbox h-5 w-5 text-blue-600" value="1"
                    {{ $post->first_section_thumbnail == 1 ? 'checked' : ''}}
                    ><span class="ml-2 text-gray-700">First Section Thumbnail</span>
                </label>
                <label class="inline-flex items-center p-2">
                    <input name="bigthumbnail" type="hidden" value="0">
                    <input name="bigthumbnail" type="checkbox" class="form-checkbox h-5 w-5 text-blue-600" value="1"
                    {{ $post->bigthumbnail == 1 ? 'checked' : ''}}
                    ><span class="ml-2 text-gray-700">Big Thumbnail</span>
                </label>
        ...
        <div class="flex p-2 mt-4">
            <div class="flex-auto flex flex-row-reverse">
                <button class="w-1/2 lg:w-1/3 border-indigo-500 bg-gray-800 text-white font-bold rounded-md px-4 py-2 m-2 transition duration-500 ease select-none hover:bg-gray-700 focus:outline-none focus:shadow-outline">Update Post</button>
            </div>
        </div>
