        ...
        <table class=" min-w-full">
            <thead>
                <tr>
                    ...
                    <th class="px-6 py-3 border-b-2 border-gray-300 text-left text-sm leading-4 text-blue-500 tracking-wider">Title EN</th>
                    <th class="px-6 py-3 border-b-2 border-gray-300 text-left text-sm leading-4 text-blue-500 tracking-wider">Title MK</th>
                    <th class="px-6 py-3 border-b-2 border-gray-300 text-left text-sm leading-4 text-blue-500 tracking-wider">Category</th>
                    <th class="px-6 py-3 border-b-2 border-gray-300 text-left text-sm leading-4 text-blue-500 tracking-wider">Area</th>
                    <th class="px-6 py-3 border-b-2 border-gray-300 text-left text-sm leading-4 text-blue-500 tracking-wider">Image</th>
                    <th class="px-6 py-3 border-b-2 border-gray-300 text-left text-sm leading-4 text-blue-500 tracking-wider">Post Created</th>
...
<tr>
        ...
        <td class="px-6 py-4 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5">{{ Str::limit($post->title_en, 10)}}</td>

        <td class="px-6 py-4 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5">{{ Str::limit($post->title_mk, 10) }}</td>

        <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-500">
            <div class="flex items-center">
                <div>
                    <div class="text-xs leading-5 text-white font-bold bg-blue-600 px-2 rounded-full">{{$post->category->category_en}}</div>
                </div>
            </div>
        </td>
        ...
        <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-500">
            <div class="flex items-center">
                <div class="">
                    <div class="text-xs leading-5 text-white font-bold bg-blue-600 px-2 rounded-full">{{$post->area->area_en}}</div>
                </div>
            </div>
        </td>
        ...
        <td class="px-4 py-2  border-b border-gray-500">
            <div class="flex items-center">
                <div class="">
                    <img src="{{ asset($post->image) }}" class="w-28  rounded-xl">
                </div>
            </div>
        </td>


        <td class="px-6 py-4 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5">{{ $post->created_at->diffForHumans() }}</td>


        <td class="whitespace-no-wrap text-right border-b border-gray-500 text-sm leading-5">
            <a href="{{ route('admin.post.edit', $post) }}"><button  type="button" class="font-bold border border-gray-500 bg-yellow-500 text-white rounded-md px-4 py-2 m-2 transition duration-500 ease select-none hover:bg-yellow-600 focus:outline-none focus:shadow-outline">Edit</button></a>
            <form class="inline-flex" action="{{ route('admin.post.destroy', $post) }}" method="POST">
                @csrf
                @method('DELETE')
