        ...
        <table class=" min-w-full">
            <thead>
                <tr>
                    ...
                    <th class="text-center py-3 border-b-2 border-gray-300 text-sm leading-4 text-blue-500 tracking-wider">Role</th>
...
<tr>
        ...
        <td class="px-6 py-4 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5">
            @if ($user->admin == 0)
                <span class=" px-3 py-1 bg-blue-500 text-white rounded-full border-1 border-blue-700 font-bold">No Role</span>
            @elseif($user->admin == 1)
             <span class=" px-3 py-1 bg-green-500 text-white rounded-full border-1 border-green-700 font-bold">Content Writer</span>
            @elseif($user->admin == 2)
            <span class=" px-3 py-1 bg-red-500 text-white rounded-full border-1 border-red-700 font-bold">Super Admin</span>
            @endif
        
        </td>
        <td class="whitespace-no-wrap text-right border-b border-gray-500 text-sm leading-5">
            <a href="{{ route('admin.role.edit', $user->id) }}"><button  type="button" class="font-bold border border-gray-500 bg-yellow-500 text-white rounded-md px-4 py-2 m-2 transition duration-500 ease select-none hover:bg-yellow-600 focus:outline-none focus:shadow-outline">Edit</button></a>
            <form class="inline-flex" action="{{ route('admin.role.destroy', $user->id) }}" method="POST">
                @csrf
                @method('DELETE')
