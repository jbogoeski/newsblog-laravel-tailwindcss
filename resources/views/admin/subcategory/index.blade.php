        ...
        <table class=" min-w-full">
            <thead>
                <tr>
                    ...
                    <th class="px-6 py-3 border-b-2 border-gray-300 text-left text-sm leading-4 text-blue-500 tracking-wider">SubCategory EN</th>
                    <th class="px-6 py-3 border-b-2 border-gray-300 text-left text-sm leading-4 text-blue-500 tracking-wider">SubCategory MK</th>
...
<tr>
        <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-500">
            <div class="flex items-center">
                <div>
                    <div class="text-sm leading-5 text-gray-800">#{{$subcategory->id}}</div>
                </div>
            </div>
        </td>
        ...
        <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-500">
            <div class="flex items-center">
                <div>
                    <div class="text-xs leading-5 text-white font-bold bg-blue-600 px-2 rounded-full">{{$subcategory->category->category_en}}</div>
                </div>
            </div>
        </td>
        <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-500">
            <div class="text-sm leading-5 text-blue-900">{{ $subcategory->subcategory_en}}</div>
        </td>
        <td class="px-6 py-4 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5">{{ $subcategory->subcategory_mk}}</td>
        <td class="whitespace-no-wrap text-right border-b border-gray-500 text-sm leading-5">
            <a href="{{ route('admin.subcategory.edit', $subcategory) }}"><button  type="button" class="font-bold border border-gray-500 bg-yellow-500 text-white rounded-md px-4 py-2 m-2 transition duration-500 ease select-none hover:bg-yellow-600 focus:outline-none focus:shadow-outline">Edit</button></a>
            <form class="inline-flex" action="{{ route('admin.subcategory.destroy', $subcategory) }}" method="POST">
                @csrf
                @method('DELETE')
...
<div class="flex items-center justify-center mx-10 my-3">
    {!! $subcategories->links('pagination::simple-tailwind') !!}
</div>
</div>
    </div>
</div>
