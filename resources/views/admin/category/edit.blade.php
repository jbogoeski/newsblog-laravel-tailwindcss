...
<div class="w-full xl:w-2/3 block px-10 bg-white rounded-xl py-5 shadow-md">
    ...
    <div class="flex justify-between py-3 mx-5">
        <span class="text-xl font-semibold text-gray-700">Update Category</span>
    ...
    <form action="{{ route('admin.category.update', $category) }}" method="POST">
        ...
        <div class="text-gray-700 md:flex md:items-center p-4">
            ...
            <div class="md:w-2/3 md:flex-grow">
                <input value="{{ $category->category_en }}" name="category_en" class="w-full h-10 px-3 text-sm placeholder-gray-600 border rounded-lg focus:shadow-outline" type="text" placeholder="Enter EN Category Name" />
                @error('category_mk')
        ...
        <div class="text-gray-700 md:flex md:items-center p-4">
            ...
            <div class="md:w-2/3 md:flex-grow">
                <input value="{{ $category->category_mk }}" name="category_mk" class="w-full h-10 px-3 text-sm placeholder-gray-600 border rounded-lg focus:shadow-outline" type="text" placeholder="Enter MK Category Name" />
                @error('category_mk')
                <span class="bg-red-500 px-3 py-1 text-xs text-white font-semibold rounded-full">{{ $message }}</span>
                @enderror
            </div>
