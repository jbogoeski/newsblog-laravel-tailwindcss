        ...
        <table class=" min-w-full">
            <thead>
                <tr>
                    ...
                    <th class="px-6 py-3 border-b-2 border-gray-300 text-left text-sm leading-4 text-blue-500 tracking-wider">Video Title EN</th>
                    <th class="px-6 py-3 border-b-2 border-gray-300 text-left text-sm leading-4 text-blue-500 tracking-wider">video Title MK</th>
...
<tr>
        <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-500">
            <div class="flex items-center">
                <div>
                    <div class="text-sm leading-5 text-gray-800">#{{$video->id}}</div>
                </div>
            </div>
        </td>
        <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-500">
            <div class="text-sm leading-5 text-blue-900">{{ $video->title_en}}</div>
        </td>
        
        <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-500">
            <div class="text-sm leading-5 text-blue-900">{{ $video->title_mk}}</div>
        </td>
        ...
        <td class="px-6 py-4 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5">
            <div class="text-sm leading-5 text-blue-900">
                @if ($video->type == 1)
                    <span class="px-3 py-1 rounded-full font-bold text-white bg-indigo-500">Big Video</span>                    
                @else
                    <span class="px-3 py-1 rounded-full font-bold text-white bg-red-500">Small Video</span>
                @endif
        ...
        <td class="whitespace-no-wrap text-right border-b border-gray-500 text-sm leading-5">
            <a href="{{ route('admin.video.edit', $video->id) }}"><button  type="button" class="font-bold border border-gray-500 bg-yellow-500 text-white rounded-md px-4 py-2 m-2 transition duration-500 ease select-none hover:bg-yellow-600 focus:outline-none focus:shadow-outline">Edit</button></a>
            <form class="inline-flex" action="{{ route('admin.video.destroy', $video->id) }}" method="POST">
                @csrf
                @method('DELETE')
